...
static inline wbImport_t wbImport_open(const char *file,
    ...
    if (file == nullptr) {
        wbLog(ERROR, "Go NULL for file value.");wbExit();
    }

    if (!wbFile_existsQ(file)) {
        wbLog(ERROR, "File ", file, " does not exist.");wbExit();
    }
    ...
    if (kind == wbImportKind_raw) {
        ...
        wbLog(ERROR, "Invalid import type.");wbExit();
    }
...
static inline wbImport_t wbImport_open(const char *file,
    ...
    if (wbString_sameQ(type, "csv")) {
        ...
        wbLog(ERROR, "Invalid import type ", type0);wbExit();
    }
